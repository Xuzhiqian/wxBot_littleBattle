...
class TulingWXBot(WXBot):
    ...
    def get_at_user_id(self, _msg):
        pid = ''
        for detail in _msg['content']['detail']:
            if detail['type']=='at':
                for user in self.m.players.values():
                    if 'remark_name' in user.info and user.info['remark_name']==detail['value']:
                        pid=user.id
                        break
                    if 'nickname' in user.info and user.info['nickname']==detail['value']:
                        pid=user.id
                        break
                    if 'display_name' in user.info and user.info['display_name']==detail['value']:
                        pid=user.id
                        break
                if pid!='':
                    break
